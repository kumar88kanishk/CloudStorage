 ...
 * @example
 * const responseHelper = require('@magcentre/response-helper');
 */
 ...
 *
 * @function sendError
 * @requires @magcentre/logger-helper
 * @category sync
...
const sendError = (error, response, otherErrorStatusCode, options, logLevel) => {
  ...
  const { method, operationId } = options.swagger.operation;
  const path = options.swagger.operation.pathObject.path;
  const correlationId = options.headers['x-correlation-id'];
  const log = (level) => {
    logger[level](`Error responding ${statusCode} for ${method} ${path} ${error}`, {
      statusCode, method, path, operationId, error, correlationId,
    }, correlationId);
  };
  if (logLevel !== true) {
 ...
 * @function sendResult
 * @requires @magcentre/logger-helper
 * @requires @magcentre/lib-filters
 * @category sync
...
const sendResult = (result, statusCode, response, options, logLevel) => {
  ...
  const log = (level) => {
    logger[level](`Responding ${statusCode} for ${method} ${path}`, {
      statusCode, result: result, method, path, operationId, correlationId,
    }, correlationId);
  };
